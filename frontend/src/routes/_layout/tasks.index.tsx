  ...
  return (
    ...
    >
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-2 px-4 py-3.5">
        ...
        {/* title */}
        <p className="line-clamp-2 text-[14px] font-semibold leading-snug text-slate-800">
          {task.name}
        </p>
        {/* deadline + progress */}
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-1 items-center gap-2">
            ...
            <span className="w-8 shrink-0 text-right text-[11px] font-semibold text-slate-600">
              {progress}%
            </span>
          </div>
        </div>
...
function SectionBox({
  ...
  return (
    <section className={cn("overflow-hidden rounded-2xl border", accentBorder)}>
      ...
      >
        <h2 className={cn("text-[13px] font-bold", accentText)}>
          {emoji} {title}
        </h2>
        <span
          className={cn("text-[12px] font-semibold opacity-75", accentText)}
        >
          {count} việc
        </span>
      </div>
...
function PendingApprovalsBox() {
  ...
  const leaveItems = (leaveQ.data?.data ?? []).filter(
    (r) => r.status === "pending",
  )
  ...
  const total =
    leaveItems.length + skillItems.length + quotations.length + contracts.length
  ...
  return (
    <Link
      to="/approvals"
      className="group block overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:border-blue-300 hover:shadow-md"
    >
      ...
      <div className="divide-y divide-slate-100 px-4">
          ...
          <div key={r.id} className="flex items-center gap-3 py-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[11px] text-slate-400">
                {new Date(r.start_date).toLocaleDateString("vi-VN")} ·{" "}
                {r.num_days} ngày
              </p>
            </div>
        ...
        {skillItems.slice(0, 2).map((r) => (
          <div key={r.id} className="flex items-center gap-3 py-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-[12px] font-semibold text-slate-800">
                {r.requested_skills
                  .map((s: any) => s.skill_name ?? s.skill_id)
                  .join(", ")}
              </p>
              <p className="text-[11px] text-slate-400">
                {r.requested_skills.length} kỹ năng ·{" "}
        ...
        {total > 4 && (
          <p className="py-2.5 text-center text-[11px] text-slate-400">
            +{total - 4} mục khác
          </p>
        )}
      </div>
...
function MyTasksPage() {
  ...
  const filteredItems = useMemo(
    () =>
      filterProjectId
        ? allItems.filter((r) => r.project_id === filterProjectId)
        : allItems,
  ...
  const grouped = useMemo(() => {
    const urgent = filteredItems.filter((r) => {
      ...
      return (
        cs === "overdue_critical" || cs === "overdue_local" || cs === "due_soon"
      )
    })
    const today = filteredItems.filter((r) => {
      const cs = r.task.computed_status ?? r.task.status
      if (["overdue_critical", "overdue_local", "due_soon"].includes(cs))
        return false
      const now = new Date()
