...
import { CheckCircle2, XCircle, CalendarOff, Star, FileText, ScrollText } from "lucide-react"
...
function SectionHeader({ icon, title, count, color }: {
  ...
  return (
    <div className={cn("flex items-center justify-between rounded-t-2xl px-4 py-3", color)}>
      <div className="flex items-center gap-2">
        {icon}
        <h2 className="text-[13px] font-bold">{title}</h2>
      </div>
      <span className="rounded-full bg-white/60 px-2.5 py-0.5 text-[11px] font-bold">{count} mục</span>
    </div>
  )
}

function ApproveRejectButtons({ onApprove, onReject, isPending }: {
...
function RejectForm({ onConfirm, onCancel, isPending }: {
  onConfirm: (note: string) => void; onCancel: () => void; isPending?: boolean
}) {
  ...
  return (
    <div className="space-y-2 border-t border-slate-100 bg-red-50 px-4 py-3">
      ...
      <div className="flex gap-2">
        ...
        <button onClick={onCancel} className="rounded-xl border px-3 py-1.5 text-xs text-slate-500 hover:bg-slate-50">Huỷ</button>
      </div>
    </div>
  )
}
...
function LeaveApprovalSection() {
  ...
  const rejectMut = useMutation({
    ...
    onSuccess: () => { setRejectTarget(null); qc.invalidateQueries({ queryKey: ["leave-pending"] }) },
  })

  return (
    <section>
      <SectionHeader icon={<CalendarOff className="h-4 w-4 text-teal-700" />}
        title="Nghỉ phép chờ duyệt" count={requests.length}
        color="bg-teal-100 text-teal-800" />
      {isLoading && <div className="h-20 animate-pulse rounded-b-2xl bg-slate-100" />}
      {!isLoading && requests.length === 0 && <EmptyBox label="Không có đơn nghỉ nào chờ duyệt" />}
              ...
              <div className="flex items-start justify-between gap-3 px-4 py-3">
                <div className="min-w-0">
                  ...
                  <p className="text-[12px] text-slate-500">
                    ...
                    {" "}· <span className="font-semibold text-slate-700">{req.num_days} ngày</span>
                  </p>
                  {req.reason && <p className="mt-1 text-[11px] italic text-slate-400">"{req.reason}"</p>}
                  {(req as any).decision_note && <p className="mt-1 text-[11px] text-red-500">Từ chối: {(req as any).decision_note}</p>}
                </div>
...
function MySkillRequestsSection() {
  ...
  return (
    <section>
      <SectionHeader icon={<Star className="h-4 w-4 text-violet-700" />}
        title="Yêu cầu kỹ năng của tôi" count={pending.length}
        color="bg-violet-100 text-violet-800" />
      {isLoading && <div className="h-20 animate-pulse rounded-b-2xl bg-slate-100" />}
      {!isLoading && requests.length === 0 && <EmptyBox label="Bạn chưa gửi yêu cầu kỹ năng nào" />}
      {!isLoading && requests.length > 0 && (
        <div className="divide-y divide-slate-50 overflow-hidden rounded-b-2xl border border-t-0 border-slate-100 bg-white">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {req.requested_skills.map((s, i) => (
                        <span key={i} className="rounded-full bg-violet-50 px-2 py-0.5 text-[10px] font-medium text-violet-700 ring-1 ring-violet-100">
                          {(s as any).skill_name ?? s.skill_id} · {LEVEL_LABEL[s.level] ?? `Cấp ${s.level}`}
                        </span>
                      ))}
                    </div>
                    {req.note && <p className="mt-1 text-[11px] italic text-slate-400">"{req.note}"</p>}
                  </div>
                  <span className={cn("shrink-0 rounded-full px-2.5 py-1 text-[11px] font-bold", m.bg, m.text)}>
                    {m.label}
                  </span>
                </div>
              </div>
...
function SkillApprovalSection() {
  ...
  const rejectMut = useMutation({
    ...
    onSuccess: () => { setRejectTarget(null); qc.invalidateQueries({ queryKey: ["skillRequests"] }) },
  })

  return (
    <section>
      <SectionHeader icon={<Star className="h-4 w-4 text-violet-700" />}
        title="Kỹ năng chờ duyệt" count={requests.length}
        color="bg-violet-100 text-violet-800" />
      {isLoading && <div className="h-20 animate-pulse rounded-b-2xl bg-slate-100" />}
      {!isLoading && requests.length === 0 && <EmptyBox label="Không có yêu cầu kỹ năng nào chờ duyệt" />}
      {!isLoading && requests.length > 0 && (
        <div className="divide-y divide-slate-50 overflow-hidden rounded-b-2xl border border-t-0 border-slate-100 bg-white">
          {requests.map((req) => (
            <div key={req.id}>
              <div className="flex items-start justify-between gap-3 px-4 py-3">
                <div className="min-w-0 flex-1">
                  <p className="text-[13px] font-bold text-slate-800">{req.user_name ?? "Nhân viên"}</p>
                  <p className="text-[11px] text-slate-400 mb-1">Gửi {new Date(req.created_at).toLocaleDateString("vi-VN")}</p>
                  <div className="flex flex-wrap gap-1">
                    {req.requested_skills.map((s, i) => (
                      <span key={i} className="rounded-full bg-violet-50 px-2 py-0.5 text-[10px] font-medium text-violet-700 ring-1 ring-violet-100">
                        {(s as any).skill_name ?? s.skill_id} · {LEVEL_LABEL[s.level] ?? `Cấp ${s.level}`}
...
function QuotationApprovalSection() {
  ...
  return (
    <section>
      <SectionHeader icon={<FileText className="h-4 w-4 text-amber-700" />}
        title="Báo giá chờ xử lý" count={quotations.length}
        color="bg-amber-100 text-amber-800" />
      {isLoading && <div className="h-20 animate-pulse rounded-b-2xl bg-slate-100" />}
      {!isLoading && quotations.length === 0 && <EmptyBox label="Không có báo giá nào chờ xử lý" />}
      {!isLoading && quotations.length > 0 && (
        <div className="divide-y divide-slate-50 overflow-hidden rounded-b-2xl border border-t-0 border-slate-100 bg-white">
          {quotations.map((q) => {
            const stageCfg = STAGE_CONFIG[q.current_stage]
            return (
              <Link key={q.id} to="/quotations/$quotationId" params={{ quotationId: q.id }} search={{ tab: "history" }}
                className="flex items-center gap-3 px-4 py-3 hover:bg-slate-50">
                <div className="min-w-0 flex-1">
                  <p className="truncate text-[13px] font-bold text-slate-800">{q.project_name}</p>
                  <p className="text-[11px] text-slate-400">{q.client_company_name} · #{q.quote_number}</p>
                </div>
                <span className={cn("shrink-0 rounded-full px-2.5 py-1 text-[11px] font-bold", stageCfg.badgeBg, stageCfg.badgeText)}>
                  {stageCfg.shortLabel}
                </span>
...
function ContractApprovalSection() {
  ...
  return (
    <section>
      <SectionHeader icon={<ScrollText className="h-4 w-4 text-indigo-700" />}
        title="Hợp đồng chờ phê duyệt" count={contracts.length}
        color="bg-indigo-100 text-indigo-800" />
      {isLoading && <div className="h-20 animate-pulse rounded-b-2xl bg-slate-100" />}
      {!isLoading && contracts.length === 0 && <EmptyBox label="Không có hợp đồng nào chờ duyệt" />}
        ...
        <div className="divide-y divide-slate-50 overflow-hidden rounded-b-2xl border border-t-0 border-slate-100 bg-white">
          {contracts.map((c) => (
            <Link key={c.id} to="/contracts/$contractId" params={{ contractId: c.id }}
              className="flex items-center gap-3 px-4 py-3 hover:bg-slate-50">
              <div className="min-w-0 flex-1">
                <p className="truncate text-[13px] font-bold text-slate-800">{c.contract_number}</p>
                <p className="text-[11px] text-slate-400">
                  {new Intl.NumberFormat("vi-VN").format(c.total_value)} {c.currency}
                </p>
              </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div className="border-b bg-white px-4 py-4 sm:px-6">
        <h1 className="text-xl font-extrabold tracking-tight text-slate-900">Phê duyệt</h1>
        <p className="mt-0.5 text-sm text-slate-500">Tất cả các mục đang chờ xử lý và phê duyệt</p>
      </div>
      ...
      <div className="space-y-5 px-4 py-4 sm:px-6">
        ...
        {!canLeaveApprove && !canSkillApprove && (
          <div className="rounded-2xl border border-dashed border-slate-200 bg-white p-10 text-center">
            ...
            <p className="mt-3 text-sm font-medium text-slate-500">Bạn không có quyền phê duyệt</p>
            <p className="mt-1 text-xs text-slate-400">Liên hệ quản trị viên để được cấp quyền.</p>
          </div>
        )}
      </div>
    </div>
