...
function SkillApprovalPanel() {
  ...
  const approveMut = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ["skillRequests", "pending"] }),
  })

  const rejectMut = useMutation({
    mutationFn: ({ id, note }: { id: string; note: string }) => rejectSkillRequest(id, note || undefined),
    onSuccess: () => {
  ...
  return (
    <div id="skill-approval" className="mb-6 rounded-2xl border border-amber-200 bg-amber-50 p-4">
      <div className="mb-3 flex items-center gap-2">
        <Inbox className="h-4 w-4 text-amber-700" />
        <h2 className="text-sm font-semibold text-amber-900">Yêu cầu duyệt kỹ năng</h2>
        <span className="rounded-full bg-amber-600 px-2 py-0.5 text-[10px] font-bold text-white">{requests.length}</span>
      </div>

      <div className="space-y-3">
        {requests.map((req) => (
          <div key={req.id} className="rounded-xl border bg-white p-3 shadow-sm">
            <div className="mb-2 flex items-start justify-between gap-2">
              <div>
                <p className="text-sm font-semibold text-slate-800">{req.user_name ?? "Nhân viên"}</p>
                <p className="text-[11px] text-slate-400">
                  Gửi lúc {new Date(req.created_at).toLocaleDateString("vi-VN")} · {req.requested_skills.length} kỹ năng
                </p>
              </div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {req.requested_skills.map((s, i) => (
                <span key={i} className="rounded-full border border-blue-100 bg-blue-50 px-2 py-0.5 text-[11px] text-blue-800">
