...
import { DashboardService, IncidentsService, TasksService, UsersService } from "@/client"
import {
...
function Loading() {
  return <p className="py-6 text-center text-sm text-slate-400">Đang tải chi tiết...</p>
}
...
function fmt(d?: string) {
  return d ? new Date(d).toLocaleDateString("vi-VN", { day: "2-digit", month: "2-digit" }) : "—"
}

// ── Nhân viên → công việc + kỹ năng thật ────────────────────────────────────
type UTask = { id?: string; name?: string; status?: string; computed_status?: string; end_time?: string; project_name?: string }

const LEVELS = [1, 2, 3, 4, 5]
const LEVEL_LABEL: Record<number, string> = { 1: "Cơ bản", 2: "Trung bình", 3: "Khá", 4: "Giỏi", 5: "Chuyên gia" }

type StaffMeta = { name?: string; group?: string; load?: number; status?: string }
...
const LOAD_LABEL_MAP: Record<string, string> = {
  free: "Đang rảnh", stable: "Đang làm", overloaded: "Quá tải",
}
...
const TASK_STATUS_LABEL: Record<string, string> = {
  todo: "Chờ làm", in_progress: "Đang làm", done: "Hoàn thành", overdue: "Trễ",
}

export function StaffDetailLive({ userId, meta }: { userId: string; meta?: StaffMeta }) {
  ...
  const tasks = useQuery({
    ...
    queryFn: () => DashboardService.userTasks({ userId }) as unknown as Promise<UTask[]>,
  })
  ...
  function startEdit() {
    setDraft((userSkills.data ?? []).map((s) => ({ skill_id: s.skill_id, level: s.level })))
    setEditSkills(true)
  }
  ...
  function setLevel(skillId: string, level: number) {
    setDraft((prev) => prev.map((s) => s.skill_id === skillId ? { ...s, level } : s))
  }

  if (tasks.isLoading || userSkills.isLoading) return <Loading />

  const taskList = tasks.data ?? []
  const initials = (meta?.name ?? "?").split(" ").map((w) => w[0]).slice(-2).join("").toUpperCase()
  const byCategory = (allSkills.data ?? []).reduce<Record<string, typeof allSkills.data>>((acc, s) => {
    if (!s) return acc
    ...
    <div className="space-y-4">
      ...
      {meta?.name && (
        <div className="flex items-center gap-3 rounded-xl bg-gradient-to-br from-blue-50 to-slate-50 p-4">
          ...
          <div className="min-w-0 flex-1">
            <p className="truncate text-base font-semibold text-slate-800">{meta.name}</p>
            {meta.group && <p className="text-xs text-slate-500">{meta.group}</p>}
            <div className="mt-1.5 flex flex-wrap gap-1.5">
              {meta.status && (
                <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${LOAD_BADGE[meta.status] ?? "bg-slate-100 text-slate-600"}`}>
      ...
      {pendingRequest && (
        <div className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2.5">
          <p className="text-xs font-semibold text-amber-800">⏳ Đang chờ quản lý duyệt kỹ năng</p>
          <p className="text-[11px] text-amber-600">
            Gửi lúc {new Date(pendingRequest.created_at).toLocaleDateString("vi-VN")} · {pendingRequest.requested_skills.length} kỹ năng đề xuất
          </p>
        </div>
      )}
      ...
      <div>
        <div className="mb-2 flex items-center justify-between">
          <p className="text-xs font-semibold text-slate-600">Kỹ năng ({userSkills.data?.length ?? 0})</p>
          {!editSkills && !pendingRequest && (
            <button onClick={startEdit} className="text-[11px] font-medium text-blue-600 hover:underline">
        ...
        {!editSkills ? (
          <div className="flex flex-wrap gap-1.5">
            ...
            {(userSkills.data ?? []).map((s) => (
              ...
              >
                <span className="text-xs font-medium text-blue-800">{s.skill_name}</span>
                <div className="flex gap-0.5">
                  {LEVELS.map((l) => (
                    <span key={l} className={`h-1.5 w-1.5 rounded-full ${l <= s.level ? "bg-blue-500" : "bg-blue-200"}`} />
                  ))}
                </div>
              </div>
            ))}
              ...
              <div key={cat}>
                ...
                <div className="space-y-1">
                  {(skills ?? []).map((sk) => {
                    ...
                    return (
                      <label key={sk.id} className="flex cursor-pointer items-center gap-2 rounded-lg border bg-white px-2.5 py-1.5 hover:border-blue-200">
                        <input
                          type="checkbox"
                          checked={!!selected}
                        ...
                        />
                        <span className="flex-1 text-xs text-slate-700">{sk.name}</span>
                        {selected && (
                          <select
            ...
            <div className="flex gap-2 pt-1">
              ...
              >
                {saveMut.isPending || submitMut.isPending
                  ? "Đang gửi..."
                  : isSelf ? "Gửi đề xuất" : "Lưu kỹ năng"}
              </button>
              <button
      ...
      {/* ── Công việc ── */}
      <div>
        <p className="mb-2 text-xs font-semibold text-slate-600">Công việc ({taskList.length})</p>
        <div className="space-y-1.5">
          {taskList.length ? taskList.map((t, i) => {
            ...
            return (
              <button
                key={i}
                disabled={!t.id}
                onClick={() => t.id && navigate({ to: "/tasks/$taskId", params: { taskId: t.id } })}
              ...
              >
                <div className="min-w-0 flex-1">
                  ...
                  {t.end_time && <p className="text-[10px] text-slate-400">Hạn {fmt(t.end_time)}</p>}
                </div>
                {stLabel && <span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-semibold ${stCls}`}>{stLabel}</span>}
                {t.id && <span className="shrink-0 text-slate-300">›</span>}
              </button>
            )
          }) : <p className="rounded-xl border bg-slate-50 px-3 py-4 text-center text-xs text-slate-400">Chưa có công việc.</p>}
        </div>
      </div>
    </div>
  )
...
type PTask = { id?: string; name?: string; status?: string; computed_status?: string; end_time?: string; assignee_name?: string; reported_progress_total?: number }
export function SiteDetailLive({ projectId, address }: { projectId: string; address?: string }) {
  const q = useQuery({
    queryKey: ["detail", "projectTasks", projectId],
    queryFn: () => TasksService.listProjectTasks({ projectId }) as unknown as Promise<PTask[]>,
  })
  const tasks = q.data ?? []
  return (
    <div className="pt-2">
        ...
        {q.isLoading ? <Loading /> : (
          <div className="space-y-1">
            {tasks.length ? tasks.map((t, i) => (
              <div key={t.id ?? i} className="rounded-lg bg-slate-50 px-2.5 py-1.5 text-xs">
                <div className="flex items-center justify-between">
                  <span className="truncate text-slate-700">{t.name ?? "—"}</span>
                  <span className="shrink-0 text-blue-600">{Math.round(t.reported_progress_total ?? 0)}%</span>
                </div>
                <p className="text-[11px] text-slate-400">{t.computed_status ?? t.status ?? ""}{t.assignee_name ? ` · ${t.assignee_name}` : ""}{t.end_time ? ` · Hạn ${fmt(t.end_time)}` : ""}</p>
              </div>
...
type Report = { progress_percent?: number; note?: string; created_at?: string; reporter_name?: string }
export function TaskDetailLive({ taskId, meta }: { taskId: string; meta?: { site?: string; skill?: string; deadline?: string; assignees?: string[] } }) {
  const q = useQuery({
    queryKey: ["detail", "progressReports", taskId],
    queryFn: () => TasksService.listProgressReports({ taskId }) as unknown as Promise<Report[]>,
  })
  ...
  return (
    <div className="pt-2">
      ...
      {meta?.assignees?.length ? <Row k="Nhân sự" v={meta.assignees.join(", ")} /> : null}
          ...
          <div className="space-y-1">
            {reports.length ? reports.map((r, i) => (
              <div key={i} className="rounded-lg bg-slate-50 px-2.5 py-1.5 text-xs">
                <div className="flex items-center justify-between">
                  <span className="text-slate-700">{r.note || `Cập nhật ${Math.round(r.progress_percent ?? 0)}%`}</span>
                  <span className="shrink-0 text-slate-400">{fmt(r.created_at)}</span>
                </div>
                {r.reporter_name ? <p className="text-[11px] text-slate-400">bởi {r.reporter_name}</p> : null}
              </div>
...
// ── Sự cố → chi tiết thật ───────────────────────────────────────────────────
type Inc = { title?: string; description?: string; category?: string; severity?: string; status?: string; root_cause?: string; solution?: string }
export function IncidentDetailLive({ incidentId }: { incidentId: string }) {
  const q = useQuery({
    queryKey: ["detail", "incident", incidentId],
    queryFn: () => IncidentsService.getIncident({ incidentId }) as unknown as Promise<Inc>,
  })
