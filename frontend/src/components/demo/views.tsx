...
function StaffBody({ s }: { s: Staff }) {
  ...
  const currentSite = s.currentSite
    ? SITES.find((p) => p.name === s.currentSite)
    : null
  ...
  return (
    <div className="space-y-1 pt-2">
      ...
      <Row
        k="Kỹ năng"
        v={s.skills.map((k) => `${k.name} (Cấp ${k.level})`).join(", ")}
      />
      <Row k="Giờ làm (30 ngày)" v={`${s.hours}h`} />
      <Row
        k="Tải trọng"
        v={
          <span className={s.load > 100 ? "text-red-600" : ""}>
            {s.load}%{s.load > 100 ? " · quá tải" : ""}
          </span>
        }
      />
          ...
          <button
            ...
            onClick={() =>
              open({
                ...
                subtitle: currentSite.address,
                accent: "green",
                body: <SiteBody p={currentSite} />,
              })
            }
          >
...
function staffBody(s: Staff) {
  return <StaffBody s={s} />
}
          ...
          {siteTasks.length ? (
            siteTasks.map((t) => (
              ...
              >
                ...
                <p className="text-[11px] text-slate-400">
                  {t.skill} · Hạn {t.deadline} · {t.assignees.length}/{t.need}{" "}
                  người
                </p>
              </button>
            ))
    ...
    <div className="space-y-1 pt-2">
      ...
      {t.assignees.length < t.need && (
        <p className="pt-1 text-xs font-medium text-amber-600">
          ⚠ Thiếu {t.need - t.assignees.length} người
        </p>
      )}
      ...
      <Section title="Lịch sử tiến độ">
        <div className="space-y-1">
          {[
            {
              d: "24/07",
              note: `Cập nhật ${t.progress}%`,
              by: t.assignees[0] ?? "—",
            },
            {
              d: "22/07",
              note: "Cập nhật 30% · kèm ảnh hiện trường",
              by: t.assignees[0] ?? "—",
            },
  ...
  return (
    <div className="space-y-1 pt-2">
      ...
      <p className="pt-3 text-[11px] text-slate-400">
        Bản thật: mở sự cố + nguyên nhân/giải pháp + ảnh + điều phối người.
      </p>
    </div>
  )
}
...
const STATUS_COLOR_DOT: Record<string, string> = {
  ...
  "Đang thi công": "bg-blue-500",
  planning: "bg-amber-400",
  "Lên kế hoạch": "bg-amber-400",
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex items-center gap-3 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-sm font-semibold text-amber-900">
              {understaffed} việc thiếu người · {free.length} người rảnh
            </p>
          </div>
          <Link
            to="/dispatch"
      ...
      <section className="space-y-2.5">
        <SectionLabel
          action={
            <Link to="/kpi" className="text-[11px] font-semibold text-blue-500">
              Xem KPI →
            </Link>
          }
        >
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm">
          {[...staff]
            ...
            .filter((s) => s.load >= 80 || s.load === 0)
            .map((s, i) => (
              <button
                key={s.id}
                onClick={() => openStaff(s)}
                className={cn(
                  "flex w-full items-center gap-3 px-4 py-2.5 hover:bg-slate-50",
                  i > 0 && "border-t border-slate-50",
                )}
              >
                <AvatarInitials name={s.name} size="sm" />
                <span className="w-24 shrink-0 truncate text-left text-[13px] font-medium text-slate-700">
                  {s.name}
                </span>
                <Bar
                  pct={s.load}
                ...
                <span
                  className={cn(
                    "w-9 shrink-0 text-right text-[11px] font-bold",
                    s.load >= 100
                      ? "text-red-600"
                      : s.load === 0
                        ? "text-amber-600"
          ...
          {staff.filter((s) => s.load >= 80 || s.load === 0).length === 0 && (
            <p className="px-4 py-3 text-xs text-slate-400">
              Tất cả nhân viên đang ở mức tải bình thường.
            </p>
          )}
        </div>
      </section>
      ...
      <div className="space-y-2">
        {[...groups.entries()].map(([g, members]) => {
          ...
          return (
            <details
              key={g}
              className="rounded-xl border border-slate-200 bg-white [&_summary]:cursor-pointer"
            >
              ...
              <div className="px-4 pb-3">
                <div className="mb-2 flex h-2 overflow-hidden rounded-full bg-slate-100">
                  ...
                  <div
                    ...
                    style={{
                      width: `${Math.round((over / members.length) * 100)}%`,
                    }}
                  />
                </div>
                {members.map((m) => {
...
export function MapView({
  ...
  return (
    <div className="space-y-3">
      <Header
        title="Sơ đồ vị trí dự án"
        sub="Click vào công trình để xem chi tiết"
      />
      ...
      <div className="flex flex-wrap gap-2">
        {Object.entries(byStatus).map(([status, count]) => (
          <span
            key={status}
            className="flex items-center gap-1.5 rounded-full border bg-white px-3 py-1 text-xs font-medium text-slate-700 shadow-sm"
          >
            <span
        ...
        <span className="flex items-center gap-1.5 rounded-full border bg-white px-3 py-1 text-xs text-slate-500 shadow-sm">
          ...
          {sites.reduce((a, s) => a + s.staffCount, 0)} nhân sự đang triển khai
        </span>
      </div>
        ...
        {sites.map((s) => (
          ...
          >
            ...
            <p className="px-4 text-xs text-slate-400">
              {s.staffCount} nhân sự đang thi công
            </p>
          </Card>
        ))}
      </Grid>
...
export function StaffView({
  ...
  return (
    <div className="space-y-4">
      <Header
        title="Nhân viên"
        sub="Kỹ năng, phân công và khối lượng công việc"
      />
      <div className="space-y-2">
        {staff.map((s) => {
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span
                    className={cn("shrink-0 text-[11px] font-semibold", l.text)}
                  >
                    {s.availability}
                  </span>
                </div>
                <p className="mb-1 truncate text-[11px] text-slate-400">
                  {s.group} · {s.tasks} việc · {s.hours}h
                </p>
  ...
  return (
    <div className="space-y-4">
      <Header
        ...
        sub="Quản lý nhà máy, tọa độ GPS và tiến độ"
      />
        ...
        {sites.map((p) => (
          <button
            ...
            onClick={() =>
              open({
                ...
                accent: "green",
                body: live ? (
                  <SiteDetailLive projectId={p.id} address={p.address} />
                ) : (
                  siteBody(p)
                ),
              })
          ...
          >
            ...
            <div className="flex items-center gap-2.5">
              ...
              <span className="w-9 shrink-0 text-right text-[11px] font-bold text-blue-600">
                {p.pct}%
              </span>
            </div>
          </button>
...
export function DispatchView() {
  ...
  return (
    <div className="space-y-4">
      <Header
        title="Điều phối"
        sub="Gợi ý nhân sự theo kỹ năng, tiến độ và khoảng cách"
      />
      <Card className="gap-1 border-amber-200 bg-amber-50 py-3">
        <p className="px-4 text-xs font-medium text-amber-800">
          Quy tắc phối hợp
        </p>
        <p className="px-4 text-[11px] text-amber-700">
          Hỗ trợ nhóm khác: Hàn, Máy trục vít, Hệ thống lạnh, Hệ thống điện, Lắp
          IQF
        </p>
        <p className="px-4 text-[11px] text-amber-700">
          Chỉ chuyên môn: Lắp panel, Bọc cách nhiệt
        </p>
      </Card>
      <div>
        ...
        <Card className="gap-1 py-3">
          ...
          <p className="px-4 text-xs text-slate-500">
            {DISPATCH_TASK.site} · {DISPATCH_TASK.skill}
          </p>
          <p className="px-4 text-xs font-medium text-red-600">
            Thiếu {DISPATCH_TASK.need} người
          </p>
        </Card>
      </div>
      <Grid cols={2}>
        {CANDIDATES.map((c) => (
          <Card key={c.name} className="gap-1 py-3">
            <div className="flex items-center justify-between px-4">
              ...
              <span
                className={`text-xs font-semibold ${c.availability === "Rảnh" ? "text-amber-600" : "text-slate-500"}`}
              >
                {c.availability}
              </span>
            </div>
            <p className="px-4 text-xs text-slate-500">
              Đang ở: {c.currentSite} · {c.distanceKm}km (~{c.etaMin} phút) ·
              Cấp {c.skillLevel}
            </p>
            <div className="flex items-center justify-between px-4">
              <span
                className={`text-[11px] font-medium ${impactCls[c.impact]}`}
              >
                {c.impact}
              </span>
              <span className="rounded-lg bg-blue-600 px-3 py-1 text-[11px] font-semibold text-white">
                Phân công
              </span>
            </div>
          </Card>
...
export function RepairView({
  ...
  return (
    <div className="space-y-3">
      <Header
        title="Sửa chữa"
        sub="Điều phối nhân viên tới công trình có sự cố"
      />
      <Grid cols={2}>
        {incidents.map((i) => (
          <Card
            ...
            onClick={() =>
              open({
                ...
                accent: "red",
                body: live ? (
                  <IncidentDetailLive incidentId={i.id} />
                ) : (
                  incidentBody(i)
                ),
              })
...
export function KpiView({
  ...
  return (
    <div className="space-y-5">
      <Header
        title="KPI & Báo cáo"
        sub="Đánh giá hiệu suất và cân bằng khối lượng công việc"
      />
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm">
          {[...staff]
            ...
            .map((s, i) => (
              ...
              >
                <AvatarInitials name={s.name} size="sm" />
                <span className="w-24 shrink-0 truncate text-[13px] font-medium text-slate-700">
                  {s.name}
                </span>
                <Bar
                  pct={s.load}
          ...
          <p className="px-4 py-2 text-[10px] text-slate-400 border-t border-slate-50">
            Tải trọng = giờ phân công ÷ giờ khả dụng ·{" "}
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span
                      className={cn("shrink-0 text-[11px] font-bold", l.text)}
                    >
                      {l.label}
                    </span>
                  </div>
                  <p className="mb-1.5 text-[11px] text-slate-400">
                    {s.group} · {s.hours}h · {s.tasks} việc
                  </p>
